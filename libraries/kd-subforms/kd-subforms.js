    ...
    subformTables.initialize = function(form){
        ...
        $(form.element()).find('div[uses-subform]').each(function(){
            ...
            if(subformConfig['subformChildSource'].toLowerCase() === 'ajax'){
              subformConfig['ajaxUrl'] = bundle.apiLocation() + '/kapps/' + bundle.kappSlug() + '/forms/' + subformConfig['subformSlug'] + '/submissions' + 
                                        '?include=details,values&timeline=createdAt&coreState=' + 
                                        'Draft&direction=DESC&q=createdBy="' + K('identity').username + '"';
            }
